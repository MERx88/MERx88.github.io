...
[https://school.programmers.co.kr/learn/courses/30/lessons/120896](https://school.programmers.co.kr/learn/courses/30/lessons/120913)

인수로 전달된 수만큼 잘라서 배열에 저장하는 문제

```js
function solution(my_str, n) {
  let answer = [];
  for (i = 0; i < Math.floor(my_str.length / n); i++) {
    answer.push([...my_str].splice(n * i, n).join(""));
  }
  if (my_str.length % n) {
    answer.push(
      [...my_str]
        .splice(n * Math.floor(my_str.length / n), my_str.length % n)
        .join("")
    );
  }
  return answer;
...
- for 반복문을 돌때 i++로 하나씩 도는것 뿐만아니라 변주를 줄수도 있다. 이문제도 i+=n으로 n씩 뛰어넘을수있다.
- splice는 배열에서 일부를 변화시키기 때문에 문자열에서는 사용불가능하다
- slice라는 잘라서 복사 전문 메소드가 있다.
- slice랑 splice 모두 끝지점이 배열을 넘어가면 그냥 끝까지로 인식해서 끝까지 문제없이 복사해서 반환한다.

위의 사항들을 반영하면 코드는 이렇게 된다.
