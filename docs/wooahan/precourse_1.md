...
그리고 또 기능목록과 완성된 코드를 보면서 느낀건 아 아직 javascript에 대한 숙련도가 부족하다는 것이었다. 정규표현식과 부동소숫점 오류를 잘알고있다면 해당 오류들을 미리 생각하고 구현해야겠다 생각했을것이다. 근데 숙련도가 낮으니 예측을 못하고 연쇄적으로 나오는 문제들에 대비만하고 있다는 느낌을 받았다. 그래서 조금은 더 자바스크립트와 친해져야겠다고 생각했다. 이러한 숙련도를 바탕으로 기능을 세분화하면 테스트케이스도 더 잘 작성가능하고 예기치 못한 문제들을 핸들링 할수있는 능력이 생기지 않을까?
